...
function nrelate_jquery_styles() {
	...
	// SQ if(isset($options["nsquared"]["status"]) && $options["nsquared"]["status"]){
	// 	$mp_options=get_option('nrelate_nsquared_options');
	// 	if(isset($mp_options['nsquared_display_ad']) && $mp_options['nsquared_display_ad']==true && $mp_options['nsquared_number_of_ads']>0){
	// 		wp_enqueue_script('thickbox');
	// 		wp_enqueue_style('thickbox');
	// 	}
	// }
