...
function nrelate_nsquared_popup_preview(NRELATE_NSQUARED_SETTINGS_URL,wp_root_nr, NRELATE_NSQUARED_PLUGIN_VERSION){
	...
	nr_href = 'http://api.nrelate.com/nsq_wp/' + NRELATE_NSQUARED_PLUGIN_VERSION + '/nrelate_popup_content.php';
	nr_numberrelated = document.getElementById("nsquared_number_of_posts").value;
	nr_num_ext = document.getElementById("nsquared_number_of_posts_ext").value;
	nr_r_title = document.getElementById("nsquared_title").value;
	// nr_r_show_post_title = document.getElementById("nsquared_show_post_title").checked;
	nr_r_max_char_perline = document.getElementById("nsquared_max_chars_per_line").value;
	nr_r_show_post_excerpt = document.getElementById("nsquared_show_post_excerpt").checked;
	nr_r_max_char_post_excerpt = document.getElementById("nsquared_max_chars_post_excerpt").value;
	nr_adval = document.getElementById("show_ad").checked;
	nr_num_ads = document.getElementById("nsquared_number_of_ads").value;
	nr_ads_placement = document.getElementById("nsquared_ad_placement").value;
	nr_logoval = document.getElementById("show_logo").checked;
	nr_thumbval = document.getElementById("nsquared_thumbnail").value;
	nr_thumbstyle = document.getElementById('nsquared_imagestyle').value;
	nr_textstyle = document.getElementById('nsquared_textstyle').value;
	nr_imageurl = document.getElementById("nsquared_default_image").value;
	nr_age_num = document.getElementById("nsquared_max_age_num").value;
	nr_age_frame = document.getElementById("nsquared_max_age_frame").value;
	nr_r_title = escape(nr_r_title);
	nr_thumbsize = document.getElementById("nsquared_thumbnail_size").value;
...
function checkblog(NRELATE_NSQUARED_SETTINGS_URL,NRELATE_API_URL,nr_domain,nr_admin_version){
...
function nr_iframe_reload(){
	document.getElementById('TB_iframeContent').src = nrelate_nsquared_popup_preview(nr_plugin_settings_url, nr_plugin_domain, nr_plugin_version)+'&TB_iframe=1&width=822&height=372';
	jQuery('#TB_iframeContent').unbind('load');
}

jQuery(document).ready(function($){
	$('.nrelate-thumbnail-style-prev').click(function(){
		$('#nsquared_imagestyle').val( $(this).parents('div:first').find('input:first').val() );
	});

	$('.nrelate-text-style-prev').click(function(){
		$('#nsquared_textstyle').val( $(this).parents('div:first').find('input:first').val() );
	});
	...
	$('.nrelate_preview_button').click(function(event){
		...
		if ($('#nsquared_thumbnail').val() == 'Thumbnails') {
			if ($('#nsquared_imagestyle').val() == 'none') return;
		} else {
			if ($('#nsquared_textstyle').val() == 'none') return;
		}
		
		_url = nrelate_nsquared_popup_preview(nr_plugin_settings_url, nr_plugin_domain, nr_plugin_version)+'&TB_iframe=1&width=822&height=372';
